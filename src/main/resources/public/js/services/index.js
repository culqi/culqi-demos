...
class Service {
  ...
  generateCharge = async (bodyCharges) => {
    return this.#http({ endPoint: "createCharge", data: bodyCharges });
  };

  createCustomer = async (bodyCustomers) => {
    return this.#http({ endPoint: "createCustomer", data: bodyCustomers });
  };

  createCard = async (bodyCard) => {
    return this.#http({ endPoint: "createCard", data: bodyCard });
  };

  createOrder = async (bodyOrder) => {
    return this.#http({ endPoint: "createOrder", data: bodyOrder });
  };
}
