...
import {
  ...
  createCustomerImpl,
  createCardImpl,
  generateOrderImpl, createPlanImpl, createSubscriptionImpl
...
const handledContentLoad = async () => {
  ...
  let tokenId,
    email, customerId, planId = null;

  if (paymentType === subscription) {
    const responsePlan = await createPlanImpl({});
  ...
  const handleResponse = async (statusCode, objResponse) => {
    let message = "";
    switch (statusCode) {
      case 200:
    ...
    if (objResponse.object) {
      message += ` - OPERACIÓN EXITOSA CON ID: ${objResponse.id ?? ''}`;

      if (objResponse.object === 'card' && paymentType === subscription) {
        const subscriptionResponse = await createSubscriptionImpl({
            cardId: objResponse.id,
            planId: planId,
        });
        selectors.subscriptionResponse.innerHTML = subscriptionResponse.data.id;
      }
    }
  ...
  const handleCulqiToken = async () => {
    ...
    await handleResponse(statusCode, objResponse);
  };
  ...
  const createCustomer = async () => {
    selectors.subscriptionResponse.innerHTML = spinerHtml;
  ...
  const loadCustomerExampleData = () => {
    customerInfo['email'] = "review-" + Math.floor(Date.now() / 1000) + "@culqi.com";
  ...
  const handleCreateSubscription = async (e) => {
    ...
    if (!customerId || !(/cus_test_([a-zA-Z0-9]{16})/.test(customerId))) {
    ...
    selectors.customerCustomFormElement.value = customerId;
    openCulqiForm(e);
  }

  const btnLoadCustomer = document.getElementById("loadCustomerExampleData");
  const btnCreateSubscription = document.getElementById("createSubscription");
  ...
  if (btnCreateSubscription) {
    btnCreateSubscription.addEventListener("click", async (e) => {
      updateCustomerInfo();
      const customer = await createCustomer(e);
      const customerCreatedId = customer.id;
